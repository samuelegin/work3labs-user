 ...
 *  - Pod name
 *  - Description
 *  - Roles needed (dynamic list — add/remove)
 *
 * On submit → POST /api/user/pods → redirect to /pod/[id]
 */
...
// ─── Suggested roles (quick-add chips) ───────────────────────────────────────
...
export default function CreatePodClient() {
  ...
  function handleRoleKeyDown(e) {
    ...
    if (e.key === 'Backspace' && !roleInput && roles.length) {
      setRoles(r => r.slice(0, -1))
    }
  }
    ...
    <div className="min-h-screen bg-paper" style={{ fontFamily: 'Outfit, sans-serif' }}>
      ...
      <div className="max-w-[680px] mx-auto px-5 sm:px-8 py-10 sm:py-14">
        ...
        <div className="mb-10" style={{ animation: 'up 0.5s cubic-bezier(0.22,1,0.36,1) both' }}>
          <span className="font-mono text-[10px] tracking-[0.14em] uppercase text-[#AAA] block mb-3">
            Pod Formation
          </span>
          ...
          <p className="text-[14px] font-light text-[#888] leading-relaxed max-w-[480px]">
            You'll become the Pod Admin. Define the team's scope, set the roles you need, then invite talents to fill them.
          </p>
        </div>
        ...
        >
          ...
          <div className="px-7 py-7 space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label htmlFor="pod-name" className="font-mono text-[10px] tracking-[0.12em] uppercase text-[#999]">
                  Pod name
                </label>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label htmlFor="pod-desc" className="font-mono text-[10px] tracking-[0.12em] uppercase text-[#999]">
                  Description
                </label>
                <span className="font-mono text-[10px] text-[#CCC]">{description.length}/400</span>
              </div>
              <p className="text-[11.5px] text-[#BBB] font-light mb-1.5 leading-snug">
                Explain the kind of work this pod executes. Projects will see this.
              </p>
              <textarea
                id="pod-desc"
                value={description}
                onChange={e => { setDescription(e.target.value.slice(0, 400)); setErrors(err => ({ ...err, description: null })) }}
                placeholder="e.g. Executes community growth, social strategy, and ecosystem expansion for Web3 protocols…"
                maxLength={400}
                rows={4}
            ...
            <div>
              ...
              <p className="text-[11.5px] text-[#BBB] font-light mb-2 leading-snug">
                Type a role and press Enter, or pick from suggestions below.
              </p>
              ...
              >
                {roles.map(r => (
                  <span
                    key={r}
                    className="flex items-center gap-1 font-mono text-[10.5px] text-ink border border-black/[0.12] bg-[#F4F4F2] rounded-full px-2.5 py-[3px] flex-shrink-0"
                  >
                    {r}
                    <button
                      type="button"
          ...
          {/* Admin role notice */}
          <div className="px-7 py-4 border-t border-black/[0.05] bg-[#F9F9F7] flex items-center gap-3">
            <i className="bi bi-shield-check text-green-dark text-[15px] flex-shrink-0" />
            <p className="text-[12.5px] font-light text-[#888] leading-snug">
              You'll be the <strong className="font-medium text-ink">Pod Admin</strong> — you can invite talents and manage the pod, but once a project is assigned you cannot remove members.
            </p>
          </div>
          ...
          <div className="px-7 py-6 flex items-center gap-3">
            <button
              ...
              disabled={submitting}
              className="flex items-center justify-center gap-2.5 bg-ink text-paper py-3.5 px-6 rounded-[10px] font-sans text-[14px] font-medium tracking-[-0.01em] hover:bg-[#1A1A1A] active:bg-[#111] transition-colors border-none cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              aria-busy={submitting}
            >
              {submitting
                ? <><Spinner /><span>Creating pod…</span></>
                : <><i className="bi bi-people text-[15px]" />Create pod</>
              }
            </button>
            <Link
              href="/dashboard"
